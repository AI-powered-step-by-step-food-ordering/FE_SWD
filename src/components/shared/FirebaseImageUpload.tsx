...
export default function FirebaseImageUpload({
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result: FirebaseUploadResponse = await uploadToFirebase(
        file,
        folder,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const result = await deleteFromFirebase(value);
      ...
      {showPreview && previewUrl && (
        <div className="relative w-full max-w-sm rounded-lg overflow-hidden border border-gray-200 bg-gray-50">
          <div className="relative w-full h-48">
            <Image
              src={getFirebaseThumbnail(previewUrl)}
              alt="Preview"
              fill
