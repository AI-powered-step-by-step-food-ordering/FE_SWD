...
export default function EditIngredientModal({ ingredient, categories, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    if (ingredient) {
      setForm({
        ...
        standardQuantity: ingredient.standardQuantity ?? 100,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const resp = await ingredientService.update(ingredient.id as string, form);
      if (resp?.data) {
        onSaved(resp.data);
        onClose();
      }
